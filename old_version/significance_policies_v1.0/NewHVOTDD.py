...
class NewHVOTDDPolicy(SigPolicy):
    ...
    def get_job_significance_result_for_all_datablocks(self, type_id, all_significance_state):
        ...
        for origin_acc in origin_history_accs:
            norm_history_accs.append(origin_acc) # 最开始的时候, 啥结果也没有的时候, 需要传一个1.0, 而后慢慢降低下去?
            
        # 全局量 * (局部量), 对量纲的影响是最小的
        # 这个算法主要是初始化的时候价值都为OTDD, 而后随着真实的结果出现, 
        max_history_acc = max(norm_history_accs)
        result = [
            norm_OTDDs[index] * norm_history_accs[index] / max_history_acc for index in range(len(all_significance_state))
        ]
        
        end = time.time()
        self.logger.info("type_id [{}] to datablocks significance: {} [norm_OTDDs: {}; norm_history_accs: {}], time: {}".format(
            type_id, result, norm_OTDDs, norm_history_accs, end-begin
        ))
        return result
