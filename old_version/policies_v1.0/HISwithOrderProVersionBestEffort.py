...
class HISwithOrderProVersionBestEffortPolicy(HISBasePolicy):
    def __init__(self, beta, job_sequence_all_num, seed, logger):
        super().__init__(beta, job_sequence_all_num, seed, logger)
        self._name = 'HISwithOrderProVersionBestEffortPolicy'
        self.beta = beta
        self.logger = logger
    ...
    def get_allocation(self, state, all_or_nothing_flag, enable_waiting_flag):
        ...
        if len(offline_history_job_priority_weights) + len(online_history_job_priority_weights) < all_job_sequence_num:
            ...
            if len(online_history_job_priority_weights) > all_job_sequence_num - 1:
                online_sample_indexes = np.random.choice(range(len(online_history_job_priority_weights)), all_job_sequence_num - 1, replace=False)
            else:
