...
def add_df_with_min_max(df):
    ...
    for index, row in df.iterrows():
        for key in add_columns_keys:
            ...
            # print(f" => len(success_num_str): {len(success_num_str)}")
...
print("---- df.info ----")
df_with_key.info()
max_min_ratio_groups = [0.1, 0.4, 0.7, 1.0]
policy_groups = {
...
y_label_name = "Average Significance of allocated jobs" # "Number of Allocated Jobs", "Ratio of Allocated Jobs", "Significance of all jobs", "Significance of allocated jobs"
results = [[0.0 for _ in range(len(max_min_ratio_groups))] for _ in range(len(policy_groups))] 
results_min = [[0.0 for _ in range(len(max_min_ratio_groups))] for _ in range(len(policy_groups))] 
results_max = [[0.0 for _ in range(len(max_min_ratio_groups))] for _ in range(len(policy_groups))] 

for policy_index, policy in enumerate(policy_groups):
    for group_index, max_min_ratio in enumerate(max_min_ratio_groups):
        test_job_num = int(df_with_key.loc[(policy, max_min_ratio), f"Online job num"])
        print(f"test_job_num: {test_job_num}")
        if y_label_name == "Number of Allocated Jobs":
            success_key_prefix = "Success num"
            results[policy_index][group_index] = df_with_key.loc[(policy, max_min_ratio), f"{success_key_prefix} avg"]
            results_min[policy_index][group_index] = df_with_key.loc[(policy, max_min_ratio), f"{success_key_prefix} min"]
            results_max[policy_index][group_index] = df_with_key.loc[(policy, max_min_ratio), f"{success_key_prefix} max"]
        elif y_label_name == "Number of Failed Jobs":
            failed_key_prefix = "Failed num"
            results[policy_index][group_index] = df_with_key.loc[(policy, max_min_ratio), f"{failed_key_prefix} avg"]
            results_min[policy_index][group_index] = df_with_key.loc[(policy, max_min_ratio), f"{failed_key_prefix} min"]
            results_max[policy_index][group_index] = df_with_key.loc[(policy, max_min_ratio), f"{failed_key_prefix} max"]
        elif y_label_name == "Ratio of Allocated Jobs":
            success_key_prefix = "Success num"
            results[policy_index][group_index] = df_with_key.loc[(policy, max_min_ratio), f"{success_key_prefix} avg"] / test_job_num
            results_min[policy_index][group_index] = df_with_key.loc[(policy, max_min_ratio), f"{success_key_prefix} min"] / test_job_num
            results_max[policy_index][group_index] = df_with_key.loc[(policy, max_min_ratio), f"{success_key_prefix} max"] / test_job_num
        elif y_label_name == "Average Significance of all jobs":
            avg_sig_all_job_key_prefix = "Mean Significance (ALL)"
            results[policy_index][group_index] = df_with_key.loc[(policy, max_min_ratio), f"{avg_sig_all_job_key_prefix} avg"]
            results_min[policy_index][group_index] = df_with_key.loc[(policy, max_min_ratio), f"{avg_sig_all_job_key_prefix} min"]
            results_max[policy_index][group_index] = df_with_key.loc[(policy, max_min_ratio), f"{avg_sig_all_job_key_prefix} max"]
        elif y_label_name == "Average Significance of allocated jobs":
            avg_sig_success_job_key_prefix = "Mean Significance (Success)"
            results[policy_index][group_index] = df_with_key.loc[(policy, max_min_ratio), f"{avg_sig_success_job_key_prefix} avg"]
            results_min[policy_index][group_index] = df_with_key.loc[(policy, max_min_ratio), f"{avg_sig_success_job_key_prefix} min"]
            results_max[policy_index][group_index] = df_with_key.loc[(policy, max_min_ratio), f"{avg_sig_success_job_key_prefix} max"]
        elif y_label_name == "Significance of all jobs":
            avg_sig_all_job_key_prefix = "Mean Significance (ALL)"
            results[policy_index][group_index] = df_with_key.loc[(policy, max_min_ratio), f"{avg_sig_all_job_key_prefix} avg"] * test_job_num
            results_min[policy_index][group_index] = df_with_key.loc[(policy, max_min_ratio), f"{avg_sig_all_job_key_prefix} min"] * test_job_num
            results_max[policy_index][group_index] = df_with_key.loc[(policy, max_min_ratio), f"{avg_sig_all_job_key_prefix} max"] * test_job_num
        elif y_label_name == "Significance of allocated jobs":
            avg_sig_success_job_key_prefix = "Mean Significance (Success)"
            success_key_prefix = "Success num"
            results[policy_index][group_index] = df_with_key.loc[(policy, max_min_ratio), f"{avg_sig_success_job_key_prefix} avg"] * df_with_key.loc[(policy, max_min_ratio), f"{success_key_prefix} avg"]
            results_min[policy_index][group_index] = df_with_key.loc[(policy, max_min_ratio), f"{avg_sig_success_job_key_prefix} min"] * df_with_key.loc[(policy, max_min_ratio), f"{success_key_prefix} avg"]
            results_max[policy_index][group_index] = df_with_key.loc[(policy, max_min_ratio), f"{avg_sig_success_job_key_prefix} max"] * df_with_key.loc[(policy, max_min_ratio), f"{success_key_prefix} avg"]
